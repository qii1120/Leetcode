...
public:
    ...
    int solve()
    {
        int time = 0, job_idx = -1, arr_max = 10000000, arr_min = -1, ppl_idx = -1, last = 0, min_fini=10000000;
        while (1)
        {
            this->display();
            for (int i = 0; i < req_n; i++)
            ...
            else
            {
                ...
                job_idx = -1, arr_max = 10000000, arr_min = -1, ppl_idx = -1, last =0,  min_fini = 10000000;
            }
            // cout<<time<<endl;
            for (int i = 0; i < req_n; i++)
            {
...
int main()
{
    ...
    while (cin >> n && n != 0)
    {
        senario Senario(n);
        //Senario.display();
        cout<<"Scenario "<<x++<<": All requests are serviced within \n"<<Senario.solve()<<" minutes."<<endl;
    }
    return 0;
}